...
class ParkingPtsSelection:
    ...
    def save_to_json(self):
        """Saves the selected parking zone points to a JSON file with scaled coordinates and slot ids."""
        scale_w, scale_h = self.imgw / self.canvas.winfo_width(), self.imgh / self.canvas.winfo_height()
        data = [
            {
                "slot_id": idx + 1,  # <- assign in order of marking
        ...
        self.messagebox.showinfo("Success", "Bounding boxes (with slot_id) saved to bounding_boxes.json")
...
class ParkingManagement(BaseSolution):
    ...
    def send_to_api(self, fs, es, occupied_slot_ids, available_slot_ids):
        """Sends occupancy data + slot ids to the API endpoint."""
        api_url = "https://corrinne-chivalric-subrotundly.ngrok-free.dev/api/update_vehicle_count"

        payload = {
            "lot_code": "BK1",
            "vehicle_count": fs,                    # filled spots
            "occupied_spots": occupied_slot_ids, # <- NEW
        ...
        try:
            ...
            LOGGER.info(f"Successfully sent occupancy data: {payload}")
            LOGGER.info(f"API Response: {response.json()}")
        except requests.exceptions.RequestException as e:
            LOGGER.error(f"Failed to send data to API: {e}")
            LOGGER.error(f"API Response: {response.text if 'response' in locals() and response is not None else 'No response received'}")
    
    def process_data(self, im0):
        """
        Processes the model data for parking lot management.
        Detects occupancy, draws regions + ids, updates counts, and sends occupied slot ids to API.
        """
        self.extract_tracks(im0)  # YOLO detections/tracks

        total_slots = len(self.json)
        fs, es = 0, total_slots                 # filled, empty
        occupied_ids, available_ids = [], []    # NEW

        annotator = Annotator(im0, self.line_width)

        for region in self.json:
            slot_id = region.get("slot_id", None)
            ...
            for box, cls in zip(self.boxes, self.clss):
                ...
                if dist >= 0:
                    ...
                    label_text = self.model.names[int(cls)]
                    cv2.putText(im0, label_text, (xc, yc + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2, cv2.LINE_AA)
                    rg_occupied = True
                    break
            ...
            if rg_occupied:
                ...
                if slot_id is not None:
                    occupied_ids.append(int(slot_id))
            else:
                if slot_id is not None:
                    available_ids.append(int(slot_id))

            # Draw region outline (red if occupied, green if available)
            color = self.occ if rg_occupied else self.arc
            cv2.polylines(im0, [pts_array], isClosed=True, color=color, thickness=2)

            # Draw the slot_id near the first vertex for visual reference
            if slot_id is not None and len(pts_array) > 0:
                x_text, y_text = int(pts_array[0][0][0]), int(pts_array[0][0][1]) - 6
                cv2.putText(im0, f"ID {slot_id}", (x_text, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2, cv2.LINE_AA)

        # Update occupancy info for on-screen analytics
        self.pr_info["Occupancy"], self.pr_info["Available"] = fs, es

        # Send data to the API (now includes slot ids)
        self.send_to_api(fs, es, occupied_ids, available_ids)

        # Simple analytics text
        cv2.putText(im0, f"{self.pr_info}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
